...
class SummonEntity {
  constructor(typeId, level, slotIndex, total, player) {
    ...
    const statScale = def.isUltimate ? 1.12 : 1;
  ...
  draw(ctx) {
    ...
    if (this.typeId === 'turret') {
      ...
      ctx.beginPath();
      ctx.arc(-9, -10, 5, 0, TAU);
      ctx.arc(9, -10, 5, 0, TAU);
      ctx.fill();
    } else if (this.typeId === 'mammoth') {
      ctx.fillStyle = this.def.color;
      ctx.beginPath();
      ctx.ellipse(0, 2, 20, 15, 0, 0, TAU);
      ctx.fill();
      ctx.fillStyle = '#636e72';
      ctx.fillRect(-22, 0, 10, 16);
      ctx.fillRect(12, 0, 10, 16);
      ctx.fillStyle = '#fff';
      ctx.beginPath();
      ctx.arc(16, -4, 4, 0, TAU);
      ctx.fill();
      ...
      const wing = Math.sin(this.wobble * 2) * 4;
      ctx.fillStyle = this.def.wingColor || '#e17055';
      ctx.beginPath();
      ctx.moveTo(-8, -6 - wing);
      ctx.lineTo(-22, -12 - wing);
      ctx.lineTo(-14, 2);
      ctx.closePath();
      ctx.fill();
      ctx.beginPath();
      ...
      ctx.fillStyle = this.def.color;
      ctx.beginPath();
      ctx.ellipse(0, 2, 20, 14, 0, 0, TAU);
      ctx.fill();
      ctx.fillStyle = this.def.coreColor || '#fdcb6e';
      ctx.beginPath();
      ctx.ellipse(0, 6, 10, 7, 0, 0, TAU);
      ctx.fill();
      ctx.fillStyle = '#fff';
